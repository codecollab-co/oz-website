...
Oz watches PTY output for localhost URLs (`http://localhost:3000`, `127.0.0.1`, Vite, Next, etc.) and surfaces them as a status-bar pill suggesting **Open in preview**.

<img src="/oz-website/web_preview.webp" alt="Oz web preview tab next to the editor" width="1200" />
...
For external URLs Oz renders them in a native child webview rather than an iframe, so cross-origin headers and cookie scopes behave like a regular browser navigation.
