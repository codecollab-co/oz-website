...
description: Tour the cli-ck window - tabs, sidebar, status bar, and the AI composer.
icon: Layout
---

The cli-ck window is a single React app with one window per workspace. Everything is built around tabs and a side panel.
...
The header carries the tab bar, workspace switcher (Local plus any WSL distro on Windows), and the notification bell. The bell lists every cli-ck-side and terminal-side agent. See [Notifications](/docs/features/notifications).

## Settings window

`Cmd+,` opens Settings in a separate window. Six tabs: General, Models, Themes, Shortcuts, Agents, About. Every preference cli-ck persists is here. See [Settings reference](/docs/reference/settings).
