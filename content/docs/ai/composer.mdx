...
<img src="/cli-ck-website/ai_workflow.webp" alt="cli-ck AI agent proposing edit diffs" width="1200" />
...
Type `@` and start the path. cli-ck fuzzy-matches against the workspace and inserts a chip pointing at the file. The file content is sent at submit, gated by the [secret deny-list](/docs/ai/security#secret-deny-list).

## `#handle` snippets

Type `#` to pick a saved snippet. Snippets are reusable prompt fragments stored in `cli-ck-ai-snippets.json`. Useful for repeated context: "the schema for our auth flow", "the conventions for this repo", etc.
