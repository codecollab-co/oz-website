...
description: Named, persistent chat sessions, project-level memory via cli-ck.md, and custom instructions.
icon: Notebook
---

Chats in cli-ck are organized into named **sessions** that persist across restarts. Sessions are not a single rolling buffer - each is its own conversation with its own message history.

## Sessions

The sessions store lives at `cli-ck-ai-sessions.json` in the app data directory:
...
## cli-ck.md - project memory

cli-ck loads `cli-ck.md` from the workspace root as agent memory, the same way Claude Code reads `CLAUDE.md` or other agents read `AGENTS.md`. It is just a Markdown file you keep in your repo.
...
`cli-ck.md` is loaded once per session, prepended to the agent's working context. It is the most direct way to make the agent useful in your specific codebase.
...
(The pattern cli-ck itself uses in its own repo.)
...
Project rules go in `cli-ck.md`. Personal rules go in custom instructions.

## Snippets, todos

Two other persisted stores live alongside sessions:

- `cli-ck-ai-snippets.json` - the `#handle` snippet library
