...
Secondary (confirmed, lower priority): privacy/local-first developers evaluating Oz specifically because it can run fully offline (LM Studio) or BYOK with no telemetry/accounts; and general AI-coding-assistant shoppers comparing Oz against tools like Cursor, Copilot, or Warp, for whom terminal-first is one factor among several.

## Product Purpose

Oz (the product, in a separate repo `my-oz/oz`) is a desktop app combining a terminal, code editor, file explorer, live web preview, and AI agents in a single ~7 MB binary for macOS, Linux, and Windows. This repo (`oz-website`) is the marketing/docs site for it — no backend, statically exported.

Success for the site: a terminal-native developer lands, understands Oz is fast/small/AI-native (not Electron-bloated), and downloads or reads docs. Success for the product: the app is genuinely usable as a plain terminal with AI available but never mandatory.
...
- Site is statically exported (`output: 'export'`) and hosted under a GitHub Pages subpath (`/oz-website`), per `next.config.mjs` and `CLAUDE.md` — README's "Deployed on Vercel" line appears stale against this; flag rather than trust if it matters to a task.
...
- Free and open source, Apache-2.0, both the app (`my-oz/oz`) and this website (`my-oz/oz-website`).
- No accounts, no hosted backend, no telemetry for the product itself.
- AI is fully optional — Oz is usable as a plain terminal with zero AI configured.
...
- Name: **Oz**. Tagline: "Terminal-first AI-native dev workspace" / "The AI-native terminal/editor/agentic env./everything."
...
- Company/org framing: CodeCollab (org name `my-oz` on GitHub) — keep as the collective identity, not individual credits.
